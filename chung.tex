...
Seeking to disprove Aristotle’s idea that an object’s speed in free fall is proportional to its weight \cite{aristotle:physics}, Galileo experimented with dropping objects from inclined planes \cite{galilei:1638:discorsi, machamer:2021:galileo}. Through these experiments, he showed that in the absence of air resistance, all objects released from rest will fall at the same downward acceleration of $g=\qty{9.8}{\meter\per\second\squared}$, independent of mass \cite{barth:2020:galileo, tipler, openstax, barrons}. \textbf{Did Galileo really measure 9.8?! Really?!}

We use the below equations when the object is in free fall, under the assumption that air resistance is negligible, acceleration is constant, and there is no initial velocity \cite{tipler, openstax, barrons}. If an object is released from rest and travels vertically under constant acceleration, its displacement is given by
\begin{equation}
%d=1/2gt^2 (Eq. 1)
...
Statistical analysis \cite{starnes:2015:practice} was performed in R \cite{R} using the \texttt{ggplot2} and \texttt{dplyr} libraries \cite{ggplot2, dplyr}. 
